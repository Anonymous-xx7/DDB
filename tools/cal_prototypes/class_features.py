...
class ClassFeatures:
    ...
    def process_label(self, label):
        ...
        pred1 = torch.zeros(batch, self.class_numbers + 1, w,
                            h).to(self.dev)
        idx = torch.where(label < self.class_numbers, label,
                          torch.Tensor([self.class_numbers]).to(self.dev))
        pred1 = pred1.scatter_(1, idx.long(), 1)
        return pred1

    def update_objective_SingleVector(self,
                                      idx,
                                      vector,
                                      name='moving_average',
                                      start_mean=True):
        ...
        if name == 'moving_average':
            self.objective_vectors[
                idx] = self.objective_vectors[idx] * self.proto_momentum + (
                    1 - self.proto_momentum) * vector.squeeze()
            self.objective_vectors_num[idx] += 1
            self.objective_vectors_num[idx] = min(
                self.objective_vectors_num[idx], 3000)
        elif name == 'mean':
            self.objective_vectors[idx] = self.objective_vectors[
                                              idx] * self.objective_vectors_num[idx] + vector.squeeze()
            self.objective_vectors_num[idx] += 1
            self.objective_vectors[idx] = self.objective_vectors[
                                              idx] / self.objective_vectors_num[idx]
            self.objective_vectors_num[idx] = min(
                self.objective_vectors_num[idx], 3000)
            pass
        else:
