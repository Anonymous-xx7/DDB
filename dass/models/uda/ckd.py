...
class CKD(UDADecorator):
    ...
    def update_objective_vector(self,
                                idx,
                                vector,
                                mode='moving_average',
                                start_mean=True,
        ...
        if vector.sum().item() == 0:
            return
        if start_mean and getattr(self,
                                  f'{name}_vectors_num')[idx].item() < 100:
            mode = 'mean'
        if mode == 'moving_average':
            getattr(self, f'{name}_vectors')[idx] = getattr(
                self, f'{name}_vectors')[idx] * self.proto_momentum + (
            ...
            getattr(self, f'{name}_vectors'
                    )[idx] = getattr(self, f'{name}_vectors')[idx] * getattr(
                self, f'{name}_vectors_num')[idx] + vector.squeeze()
            getattr(self, f'{name}_vectors_num')[idx] += 1
            getattr(self, f'{name}_vectors')[idx] = getattr(
                self, f'{name}_vectors')[idx] / getattr(
                self, f'{name}_vectors_num')[idx]
            getattr(self, f'{name}_vectors_num')[idx] = min(
                getattr(self, f'{name}_vectors_num')[idx], 3000)
            pass
        else:
    ...
    def process_label(self, label):
        ...
        idx = torch.where(label < self.num_classes, label,
                          torch.Tensor([self.num_classes]).cuda())
    ...
    def feat_proto_distance(self, feature: torch.Tensor, name='stu'):
        ...
        for i in range(self.num_classes):
            feat_proto_distance[:, i, :, :] = torch.norm(
                getattr(self, f'{name}_vectors')[i].reshape(-1, 1, 1).expand(
                    -1, h, w) - feature,
                2,
                dim=1,
            )
    ...
    def _init_sub_model_weights(self):
        ...
        if self.proto_rectify:
            self.cu_vectors = torch.load(
                self.cu_proto_path, map_location='cpu').cuda()
            self.ca_vectors = torch.load(
                self.ca_proto_path, map_location='cpu').cuda()
    ...
    def forward_train(self, img, img_metas, gt_semantic_seg, target_img,
        ...
        if not self.soft_distill:
            with torch.no_grad():
                if self.proto_rectify:
                    cu_tgt_logits, cu_tgt_feature = self.encode_decode(target_img, target_img_metas,
                                                                       'cu', return_feature=True)
                    ca_tgt_logits, ca_tgt_feature = self.encode_decode(target_img, target_img_metas,
                                                                       'ca', return_feature=True)
                    ...
                    cu_tgt_logits = self.encode_decode(target_img, target_img_metas,
                                                       'cu')
                    ca_tgt_logits = self.encode_decode(target_img, target_img_metas,
                                                       'ca')
                    cu_tgt_softmax = torch.softmax(cu_tgt_logits, dim=1)
                    ca_tgt_softmax = torch.softmax(ca_tgt_logits, dim=1)
                    _, cu_tgt_pl = torch.max(cu_tgt_softmax, dim=1)
                    _, ca_tgt_pl = torch.max(ca_tgt_softmax, dim=1)
                ensemble_tgt_softmax = cu_tgt_softmax.detach() * 0.5 + \
                                       ca_tgt_softmax.detach() * 0.5
            ...
            stu_tgt_losses = self.get_model('stu').forward_train(
                stu_tgt_img, target_img_metas, tgt_pseudo_label.unsqueeze(1), pseudo_weight,
            ...
            ensemble_tgt_softmax = torch.softmax((cu_tgt_logits + ca_tgt_logits) / 2 / self.temp, dim=1)
            stu_tgt_losses = kl_loss(stu_tgt_softmax,
                                     ensemble_tgt_softmax.detach()) * self.soft_distill_w * self.temp ** 2

            stu_tgt_loss, stu_tgt_log_vars = self._parse_losses({'stu_tgt.loss_distill': stu_tgt_losses})
        ...
        if self.local_iter % self.debug_img_interval == 0:
            ...
            vis_tgt_img = torch.clamp(denorm(stu_tgt_img, means, stds), 0, 1)
            for j in range(batch_size):
                ...
                if not self.soft_distill:
                    subplotimg(
                        ...
                        cmap='cityscapes')
                    subplotimg(
                        axs[0][2], cu_tgt_pl[j], 'Cu. Seg', cmap='cityscapes')
                    subplotimg(
                        axs[1][2], ca_tgt_pl[j], 'Ca. Seg', cmap='cityscapes')
                    if self.proto_rectify:
                        subplotimg(
                            axs[0][3],
                            cu_rectify_tgt_pl[j],
    ...
    def inference(self,
        ...
        """
        return self.get_model(name).inference(img, img_meta, rescale,
                                              return_seg_logit)
